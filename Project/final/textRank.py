...
abstractPath = '/home/palash/Downloads/7th-Sem/NLP/datasets/NIPS-Papers/final/attempt2-textrank/'

for file in os.listdir(paperPath):
    print file
    pFile = paperPath + file
    f= open(pFile, "r")
    text = f.read()
    a = extractSentences(text)
    aFile = abstractPath + file.replace('p','a')
    f1 = open(aFile, "w")
    f1.write(a)
    f1.close()